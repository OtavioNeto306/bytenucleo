...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($subscription->slugExists($slug, $planId)) {
        $errors[] = 'Já existe um plano com este nome';
    }
    
    // Se não há erros, atualizar o plano
    if (empty($errors)) {
        $planData = [
            'name' => $name,
            'slug' => $slug,
        ...
        if ($subscription->updatePlan($planId, $planData)) {
            header('Location: planos.php?success=updated');
            exit;
        } else {
            $errors[] = 'Erro ao atualizar plano. Tente novamente.';
        }
    }
}
...
// Obter features atuais do plano
         ...
         <section class="py-80 bg-primary-50">
             <div class="container">
                 <div class="row align-items-center">
                     <div class="col-lg-8">
                         <h1 class="display-4 fw-bold mb-24">Editar Plano</h1>
                                                   <p class="text-lg text-neutral-600 mb-0">
                              Edite as informações do plano "<?= htmlspecialchars($plan['name']) ?>"
                          </p>
                     </div>
                            ...
                            <div class="card-body p-32">
                                ...
                                <form method="POST">
                                    ...
                                    <div class="mb-32">
                                        ...
                                        <div class="row g-16">
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" class="form-control bg-base border-secondary" 
                                                       name="name" value="<?= htmlspecialchars($_POST['name'] ?? $plan['name']) ?>" 
                                                       placeholder="Ex: Plano Premium" required>
                                            </div>
                                            <div class="col-md-6">
                                                                                                 <label class="form-label">Status</label>
                                                <select class="form-select bg-base border-secondary" name="status">
                                                    <option value="active" <?= ($_POST['status'] ?? $plan['status']) === 'active' ? 'selected' : '' ?>>Ativo</option>
                                                    <option value="inactive" <?= ($_POST['status'] ?? $plan['status']) === 'inactive' ? 'selected' : '' ?>>Inativo</option>
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mt-16">
                                            ...
                                            <textarea class="form-control bg-base border-secondary" name="description" 
                                                      rows="3" placeholder="Descreva os benefícios deste plano" required><?= htmlspecialchars($_POST['description'] ?? $plan['description']) ?></textarea>
                                        </div>
                                    </div>
                                        ...
                                        <div class="row g-16">
                                            <div class="col-md-6">
                                                ...
                                                <input type="number" class="form-control bg-base border-secondary" 
                                                       name="price" value="<?= htmlspecialchars($_POST['price'] ?? $plan['price']) ?>" 
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <input type="number" class="form-control bg-base border-secondary" 
                                                       name="duration_days" value="<?= htmlspecialchars($_POST['duration_days'] ?? $plan['duration_days']) ?>" 
                                                       min="1" required>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-32">
                                        ...
                                        <div class="row g-16">
                                            <div class="col-md-6">
                                                ...
                                                <input type="number" class="form-control bg-base border-secondary" 
                                                       name="max_downloads" value="<?= htmlspecialchars($_POST['max_downloads'] ?? ($plan['max_downloads'] == -1 ? '' : $plan['max_downloads'])) ?>" 
                                             ...
                                             <div class="col-md-6">
                                                ...
                                                <input type="number" class="form-control bg-base border-secondary" 
                                                       name="max_products" value="<?= htmlspecialchars($_POST['max_products'] ?? ($plan['max_products'] == -1 ? '' : $plan['max_products'])) ?>" 
                                                       min="-1" placeholder="Deixe vazio para ilimitado">
                                                                                                 <small class="text-neutral-600">Use -1 para ilimitado</small>
                                            </div>
                                        </div>
                                    ...
                                    <div class="mb-32">
                                        ...
                                        } elseif (is_array($currentFeatures)) {
                                    ...
                                    <div class="d-flex gap-16">
                                        <button type="submit" class="btn btn-primary">
                                            <i class="ri-save-line me-2"></i>Atualizar Plano
                                        </button>
